...
BASE_CONFIG="${SENSOR_PROTO_RECORD_TEMPLATE_CONFIG:-configs/hikrobot-stream.json}"
GENERATED_CONFIG="${SENSOR_PROTO_RECORD_GENERATED_CONFIG:-artifacts/hikrobot-stream-recording-runtime.json}"
OUTPUT_DIR="${SENSOR_PROTO_RECORD_OUTPUT_DIR:-artifacts/lerobot/hik-${DURATION_S}s-episode-${TIMESTAMP}}"
REPO_ID="${SENSOR_PROTO_RECORD_REPO_ID:-local/sensor-proto-hik}"
...
# Stop any running stream service so the USB cameras are free for recording.
docker compose -f docker/compose.yaml --profile hikrobot stop sensor-hikrobot-stream 2>/dev/null || true

mkdir -p "${REPO_ROOT}/artifacts/lerobot"
TEMP_CONFIG="$(mktemp "${REPO_ROOT}/artifacts/hikrobot-stream-recording-XXXXXX.json")"
...
set +e
docker compose -f docker/compose.yaml --profile hikrobot run --rm --service-ports --entrypoint bash sensor-hikrobot-stream -lc \
