...
class StreamAutoConfigTests(unittest.TestCase):
    ...
    def test_build_realsense_stream_config_payload_replaces_camera_inventory(self) -> None:
        ...
        self.assertFalse(payload["transport"]["enabled"])
        self.assertEqual(payload["transport"]["kind"], "zmq")
        self.assertEqual(payload["transport"]["max_queue"], 1)
        self.assertEqual(payload["transport"]["backpressure_strategy"], "latest_only_drop_oldest")
